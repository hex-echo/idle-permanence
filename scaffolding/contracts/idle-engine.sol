...
contract Idle {
    ...
    function changeAction(uint16 _action_id) public {
        ...
        PlayerData storage player_data = players[msg.sender];
        //require(player_data.player_addr != address(0),"This address has not been stored before");
        if(player_data.curr_player_action_id == action_id_woodcutting){
            actionWoodcutting(_action_id);
        }else if(player_data.curr_player_action_id == action_id_mining){
            actionMining(_action_id);
        }else if(player_data.curr_player_action_id == action_id_firemaking){
            actionFiremaking(_action_id);
        }else if(player_data.curr_player_action_id == action_id_smithing){
    ...
    function getBalance() public view returns (uint256 curr_balance) {
        PlayerData storage player_data = players[msg.sender];
        return player_data.player_moonies[0];
    }
    function getManagerCount() public view returns (uint256 num_managers) {
        PlayerData storage player_data = players[msg.sender];
        return player_data.player_moonies[1];
    }
    function getManagerFactor() public view returns (uint256 factor) {
        PlayerData storage player_data = players[msg.sender];
        return player_data.player_moonies[2];
    }
    function updateBalance() public returns (uint256 curr_balance){
        PlayerData storage player_data = players[msg.sender];
        player_data.player_moonies[0] += player_data.player_moonies[1] * player_data.player_moonies[2] * (block.timestamp - player_data.last_timestamp);
    ...
    function init() public {
        ...
        player_data.player_upgrades = [1,1,1,1,1,1,1,1];
        player_data.last_timestamp = block.timestamp;
    ...
    function getUpgrades() public view returns (uint256[8] memory upgrades) {
        PlayerData storage player_data = players[msg.sender];
        //require(player_data.player_addr == address(0), "This address has not been stored before");
        return player_data.player_upgrades;
    }
    function getCurrentActionID() public view returns (uint16 action_id) {
        PlayerData storage player_data = players[msg.sender];
        //require(player_data.player_addr == address(0), "This address has not been stored before");
        return player_data.curr_player_action_id;
    }
    //action functions currently only account for 1 type of resource accumulation: 1 log (moonie_ID: 1)
    function actionWoodcutting(uint16 _action_id) public returns (uint256 log_balance){
        PlayerData storage player_data = players[msg.sender];
        setActionState(_action_id);
    ...
    function sellMoonies(uint _moonie_id, uint _amt_sold) public returns (uint sale_amt)
    {
        ...
        setMoonie(_moonie_id, player_data.player_moonies[_moonie_id] - _amt_sold);
        setMoonie(0, player_data.player_moonies[0] + sale);
        return sale;
     }
